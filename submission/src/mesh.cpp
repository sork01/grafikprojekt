...
void Mesh::getWireframe(vector<WireframeElement>& stuff)
{
    for (Triangle t : triangles)
    {
        ...
        // add the vertices
        e.points.push_back(t.vertex[0].position);
        e.points.push_back(t.vertex[1].position);
        e.points.push_back(t.vertex[2].position);
 ...
 */
bool Mesh::getIntersection(vec3 start, vec3 dir, vec3 dir_inv, Intersection& is, BoundingBox* box)
{
    bool result = false;
    ...
    if (box->hasSubs)
    {
        ...
        if (box->sub[0] != nullptr && getIntersection(start, dir, dir_inv, subis, box->sub[0]))
        {
            is = subis;
            result = true;
        ...
        if (box->sub[1] != nullptr && getIntersection(start, dir, dir_inv, subis, box->sub[1]) && subis.distance < min_distance)
        {
            is = subis;
            result = true;
        }
    ...
    else // no subboxes, test triangles in current box
    {
        ...
        for (Triangle* t : box->triangles)
        {
            ...
            vec3 b = start - t->vertex[0].position;
            mat3 A(-dir, e1, e2);
        ...
        if (result)
        {
            vec3 bc = computeBarycentricCoords(is.position, tf->vertex[0].position, tf->vertex[1].position,
                tf->vertex[2].position);
...
void Mesh::cullInvisibleParts(vec3 cameraPosition)
{
    for (Triangle& t : triangles)
    {
        ...
        if (glm::dot(glm::normalize(cameraPosition - t.vertex[0].position), t.geometricNormal) < 0.0f)
        {
            t.hidden = true;
        }
        else
...
{
    temporaryVertices[numSetTemporaryVertices++ % 3] = Vertex(position, normal);
}
...
{
    temporaryVertices[numSetTemporaryVertices++ % 3] = Vertex(position, normal, textureCoords);
}
...
{
    triangles.push_back(
        Triangle(temporaryVertices[0], temporaryVertices[1], temporaryVertices[2], mat));
    
    numSetTemporaryVertices = 0;
}
...
void Mesh::flipY()
{
    ...
    for (Triangle& t : triangles)
    {
        ...
        t.geometricNormal.y = -t.geometricNormal.y;
    }
}
...
void Mesh::computeBVH()
{
    ...
    for (Triangle& t : triangles)
    {
        mainbox.triangles.push_back(&t);
        
        for (int i = 0; i < 3; ++i)
        {
            mainbox.min.x = glm::min(mainbox.min.x, t.vertex[i].position.x);
            mainbox.min.y = glm::min(mainbox.min.y, t.vertex[i].position.y);
            mainbox.min.z = glm::min(mainbox.min.z, t.vertex[i].position.z);
            
            mainbox.max.x = glm::max(mainbox.max.x, t.vertex[i].position.x);
            mainbox.max.y = glm::max(mainbox.max.y, t.vertex[i].position.y);
            mainbox.max.z = glm::max(mainbox.max.z, t.vertex[i].position.z);
        }
    }
    
    // inflate the main bounding box slightly if it has zero size in some dimension
    if (mainbox.min.x == mainbox.max.x)
    {
        mainbox.min.x -= 0.1f;
        mainbox.max.x += 0.1f;
    }
    
    if (mainbox.min.y == mainbox.max.y)
    {
        mainbox.min.y -= 0.1f;
        mainbox.max.y += 0.1f;
    }
    
    if (mainbox.min.z == mainbox.max.z)
    {
        mainbox.min.z -= 0.1f;
        mainbox.max.z += 0.1f;
    }
...
void Mesh::BVH(BoundingBox* box, BVHCut cut, uint depth)
{
    ...
    // abort recursion
    if (depth > 5 || (depth > 0 && box->triangles.size() < 25))
    {
        return;
    }
